...
const NAV_CONFIG = {
  organization: {
    ...
    items: [
      { id: 'org-dashboard', label: 'Org dashboard', icon: 'layout-dashboard', active: true },
      { 
        id: 'accounts', 
        label: 'Accounts', 
        icon: 'building-bank',
        ...
        children: [
          ...
          { id: 'campaign-approval', label: 'Campaign approval' }
        ]
      },
...
function createNavItem(item, isChild = false) {
    ...
    return `
      <li>
        ...
        <ul id="${item.id}-children" class="sidenav-children ${item.expanded ? '' : 'hidden'} mt-1 space-y-0.5">
          ${item.children.map(child => createNavItem(child, true)).join('')}
        </ul>
      </li>
    `;
  }
  ...
  if (isChild) {
    return `
      <li>
        <a href="#" class="${baseClasses} ${activeClasses}">
...
function renderLeftNav(containerId) {
  ...
  if (sidebar) {
    sidebar.classList.add('group', 'sidenav-expanded');
  }
  ...
  container.innerHTML = `
    ...
    >
      <svg class="sidenav-collapse-icon h-5 w-5 transition-transform" stroke="currentColor" fill="none" stroke-width="1.5" viewBox="0 0 24 24" stroke-linecap="round" stroke-linejoin="round">
    ...
    <div class="flex-1 overflow-y-auto overflow-x-hidden custom-scrollbar">
      ...
      <!-- Navigation Sections -->
      <nav class="px-2 pt-3">
        ${createSection(NAV_CONFIG.organization)}
        ${createSection(NAV_CONFIG.marketing)}
      </nav>
    </div>
  `;
}
